            ...
            if (/\.js$/.test(filename)) {
                try {
                    ...
                    for (let [k, v] in Iterator(e))
                        err[k] = v;
                    err.echoerr = xml({raw: ["", ":", ": ", ""],cooked: ["", ":", ": ", ""]}, [(file.path), (e.lineNumber), (e)]);
                    throw err;
                }
            }
    ...
    commands: function () {
        ...
        commands.add(["!", "run"],
            ...
            function (args) {
                ...
                commandline.echo(template.genericOutput("Command Output: " + arg, xml({raw: ["<span highlight=\"CmdOutput\">", "</span>"],cooked: ["<span highlight=\"CmdOutput\">", "</span>"]}, [(String(output))])));
