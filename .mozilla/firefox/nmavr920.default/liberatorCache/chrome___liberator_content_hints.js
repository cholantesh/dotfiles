...
const Hints = Module("hints", {
    ...
    _generate: function (win, screen) {
        ...
        let baseNodeAbsolute = util.xmlToDom(xml({raw: ["<span highlight=\"Hint\"/>"],cooked: ["<span highlight=\"Hint\"/>"]}, []), doc);

        let res = util.evaluateXPath(this._hintMode.tags(), doc, null, true);

        let fragment = util.xmlToDom(xml({raw: ["<div highlight=\"hints\" style=\"position:fixed;top:0;left:0px;\"/>"],cooked: ["<div highlight=\"hints\" style=\"position:fixed;top:0;left:0px;\"/>"]}, []), doc);
            ...
            for (let i in (util.interruptibleRange(start, end + 1, 500))) {
                ...
                if (hint.text == "" && hint.elem.firstChild && hint.elem.firstChild instanceof HTMLImageElement) {
                    if (!hint.imgSpan) {
                        ...
                        hint.imgSpan = util.xmlToDom(xml({raw: ["<span highlight=\"Hint\" liberator:class=\"HintImage\" xmlns:liberator=", "/>"],cooked: ["<span highlight=\"Hint\" liberator:class=\"HintImage\" xmlns:liberator=", "/>"]}, [(NS)]), doc);
