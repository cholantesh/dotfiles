...
const Buffer = Module("buffer", {
        ...
        this.addPageInfoSection("f", "Feeds", function (verbose) {
            ...
            for (let link in util.evaluateXPath(["link[@href and (@rel='feed' or (@rel='alternate' and @type))]"], doc)) {
                ...
                if (isValidFeed(feed, doc.nodePrincipal, rel == "feed")) {
                    ...
                    if (verbose)
                        yield [feed.title, xml({raw: ["", "<span class=\"extra-info\">&#xa0;(", ")</span>"],cooked: ["", "<span class=\"extra-info\">&#xa0;(", ")</span>"]}, [(template.highlightURL(feed.href, true)), (type)])];
                }
            }
    ...
    shiftFrameFocus: function (count, forward) {
        ...
        let indicator = util.xmlToDom(xml({raw: ["<div highlight=\"FrameIndicator\"/>"],cooked: ["<div highlight=\"FrameIndicator\"/>"]}, []), doc);
    ...
    showElementInfo: function (elem) {
        liberator.echo(xml({raw: ["Element:<br/>", ""],cooked: ["Element:<br/>", ""]}, [(util.objectToString(elem, true))]), commandline.FORCE_MULTILINE);
    },
    ...
    showPageInfo: function (verbose, sections) {
        ...
        if (!verbose) {
            ...
            let pageInfoText = xml({raw: ["", " [", "] ", ""],cooked: ["", " [", "] ", ""]}, [(file.quote()), (info), (title)]);
    ...
    completion: function () {
        ...
        completion.buffer = function buffer(context, flags) {
            ...
            context.process = [function (item, text)
                    xml({raw: ["\n\
                        <span highlight=\"Indicator\" style=\"display: inline-block; width: 2ex; padding-right: 0.5ex; text-align: right\">", "</span>\n\
                        ", "\n\
                    "],cooked: ["                        <span highlight=\"Indicator\" style=\"display: inline-block; width: 2ex; padding-right: 0.5ex; text-align: right\">", "</span>                        ", "                    "]}, [(item.item.indicator), ( process.call(this, item, text) )])];
