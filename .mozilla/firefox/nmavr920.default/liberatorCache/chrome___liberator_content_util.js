...
const Util = Module("util", {
                ...
                switch(node.nodeType) {
                    case Node.ELEMENT_NODE:
                        ...
                        for (let { name: name, value: value } in util.Array.itervalues(node.attributes)) {
                            ...
                            data.push(name);
                            data.push('="');
                            data.push(xml({raw: ["", ""],cooked: ["", ""]}, [(value)]).toString());
                            data.push('"')
                        }
                        ...
                        break;
                    case Node.TEXT_NODE:
                        data.push(xml({raw: ["", ""],cooked: ["", ""]}, [(node.textContent)]).toString());
                }
            }
    ...
    objectToString: function objectToString(object, color) {
        ...
        if (object instanceof Element) {
            ...
            try {
                let tag = xml({raw: ["", "", "", ""],cooked: ["", "", "", ""]}, [(("<" + (namespaced(elem)) + " ")), (
                    template.map2(xml, elem.attributes,
                        function (a) xml({raw: ["", "=", ""],cooked: ["", "=", ""]}, [(namespaced(a)), (template.highlight(a.value, true))]), " ")), (
        ...
        obj = template.highlightFilter(util.clip(obj, 150), "\n", !color ? function () "^J" : function () xml({raw: ["<span highlight=\"NonText\">^J</span>"],cooked: ["<span highlight=\"NonText\">^J</span>"]}, []));
        let string = xml({raw: ["<span highlight=\"Title Object\">", "</span>::<br/>&#xa;"],cooked: ["<span highlight=\"Title Object\">", "</span>::<br/>&#xa;"]}, [(obj)]);
        ...
        try { // window.content often does not want to be queried with "var i in object"
            ...
            for (let i in object) {
                let value = xml({raw: ["<![CDATA[<no value>]]>"],cooked: ["<![CDATA[<no value>]]>"]}, []);
                try {
                    value = object[i];
                }
                catch (e) {}
                ...
                let key = xml({raw: ["<span highlight=\"Key\">", "</span>"],cooked: ["<span highlight=\"Key\">", "</span>"]}, [(i)]);
                if (!isNaN(i))
                    i = parseInt(i);
                else if (/^[A-Z_]+$/.test(i))
                    i = "";
                keys.push([i, xml({raw: ["", "", "<br/>&#xa;"],cooked: ["", "", "<br/>&#xa;"]}, [(key), (noVal ? "" : xml({raw: [": ", ""],cooked: [": ", ""]}, [(value)]))])]);
            }
        }
        catch (e) {}
    ...
    xmlToDomForTemplate: function xmlToDomForTemplate(node, doc, nodes) {
        ...
        var fragment = range.createContextualFragment(
            xml({raw: ["<div xmlns:ns=", " xmlns:xul=", " xmlns=", ">", "</div>"],cooked: ["<div xmlns:ns=", " xmlns:xul=", " xmlns=", ">", "</div>"]}, [(NS), (XUL), (XHTML), (node)]).toString());
