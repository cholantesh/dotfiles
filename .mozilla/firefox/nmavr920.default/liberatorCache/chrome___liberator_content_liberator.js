...
const Liberator = Module("liberator", {
    ...
    version: "3.7.1 (created: 2013/05/22 20:32:57)", // these VERSION and DATE tokens are replaced by the Makefile
        ...
        try {
            ...
            if (str instanceof Error) {
                ...
                stackTrace = template.map2(xml, stackItems.slice(1), function (stackItema) {
                    ...
                    return xml({raw: ["<span style=\"font-weight: normal\">&#xa0;&#xa0;xxx at </span>", "<span style=\"font-weight: normal\">", "</span><br/>"],cooked: ["<span style=\"font-weight: normal\">&#xa0;&#xa0;xxx at </span>", "<span style=\"font-weight: normal\">", "</span><br/>"]}, [(stackLocation), (stackArguments)]);
                });
    ...
    initHelp: function () {
        ...
        body = xml.map([con for ([,con] in Iterator(plugins.contexts))], function (context) {
            ...
            if (!info) return "";
            var div = ps.parseFromString(xml({raw: ["<div xmlns=", ">", "</div>"],cooked: ["<div xmlns=", ">", "</div>"]}, [(XHTML), (info)]), "text/xml").documentElement;
            ...
            return xml({raw: ["<h2 xmlns=", " tag=", ">", "</h2>", ""],cooked: ["<h2 xmlns=", " tag=", ">", "</h2>", ""]}, [(NS.uri), (info.getAttribute("name") + '-plugin'), (
                info.getAttribute("summary")), (xml.raw({raw: ["", ""],cooked: ["", ""]}, [(encoder.encodeToString())]))]);
            } catch (ex) {
                Cu.reportError(ex);
                alert(ex);
            }
                ...
                if (extensions.length > 0) {
                    let list = template.tabular(
                        ...
                        ([template.icon(e, e.name),
                          e.version,
                          e.enabled ? xml({raw: ["<span highlight=\"Enabled\">enabled</span>"],cooked: ["<span highlight=\"Enabled\">enabled</span>"]}, [])
                                    : xml({raw: ["<span highlight=\"Disabled\">disabled</span>"],cooked: ["<span highlight=\"Disabled\">disabled</span>"]}, []),
                ...
                try {
                    if (count > 1) {
                        ...
                        let str = template.genericOutput("Code execution summary",
                                xml({raw: ["<table>\n\
                                    <tr><td>Executed:</td><td align=\"right\"><span class=\"times-executed\">", "</span></td><td>times</td></tr>\n\
                                    <tr><td>Average time:</td><td align=\"right\"><span class=\"time-average\">", "</span></td><td>{eachUnits}</td></tr>\n\
                                    <tr><td>Total time:</td><td align=\"right\"><span class=\"time-total\">", "</span></td><td>{totalUnits}</td></tr>\n\
                                </table>"],cooked: ["<table>                                    <tr><td>Executed:</td><td align=\"right\"><span class=\"times-executed\">", "</span></td><td>times</td></tr>                                    <tr><td>Average time:</td><td align=\"right\"><span class=\"time-average\">", "</span></td><td>{eachUnits}</td></tr>                                    <tr><td>Total time:</td><td align=\"right\"><span class=\"time-total\">", "</span></td><td>{totalUnits}</td></tr>                                </table>"]}, [(count), (each.toFixed(2)), (total.toFixed(2))]));
